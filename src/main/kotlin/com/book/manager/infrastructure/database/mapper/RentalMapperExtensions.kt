...
import com.book.manager.infrastructure.database.mapper.RentalDynamicSqlSupport.Rental.rentalDeadline
...
fun RentalMapper.insert(record: RentalRecord) =
    insert(this::insert, record, Rental) {
        ...
        map(rentalDeadline).toProperty("rentalDeadline")
    }
...
fun RentalMapper.insertSelective(record: RentalRecord) =
    insert(this::insert, record, Rental) {
        ...
        map(rentalDeadline).toPropertyWhenPresent("rentalDeadline", record::rentalDeadline)
    }

private val columnList = listOf(bookId, userId, rentalDatetime, rentalDeadline)
...
fun KotlinUpdateBuilder.updateAllColumns(record: RentalRecord) =
    apply {
        ...
        set(rentalDeadline).equalTo(record::rentalDeadline)
    }
...
fun KotlinUpdateBuilder.updateSelectiveColumns(record: RentalRecord) =
    apply {
        ...
        set(rentalDeadline).equalToWhenPresent(record::rentalDeadline)
    }
